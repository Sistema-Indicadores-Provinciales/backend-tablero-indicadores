...
4. Ingresar al sistema como ADMIN y abrir **Administración → Conexiones** desde el menú. Pegar el ID y el secreto del mismo cliente OAuth para habilitar la conexión persistente. El secreto se cifra en `analytics_settings` y nunca se devuelve al navegador; dejar el campo vacío conserva el anterior si no cambia el ID. No hace falta reiniciar. Como alternativa, configurar `GOOGLE_CLIENT_ID` y `GOOGLE_CLIENT_SECRET` en analytics; el valor guardado en el sistema tiene prioridad. `VITE_GOOGLE_CLIENT_ID` por sí solo permite únicamente el modo temporal anterior. Nunca poner secretos en variables VITE.
...
La aplicación pide `spreadsheets.readonly`, consulta por enlace y no modifica archivos. Con el secreto configurado, usa el modelo de código de autorización de Google con popup. `POST /v2/google/connect` valida Bearer, origen permitido por `CORS_ORIGINS`, cabecera `X-Requested-With: XMLHttpRequest` y cliente activo. El servidor canjea el código y guarda los tokens cifrados por ID de usuario en `analytics_google_connections`. Renueva el acceso al vencer, incluso tras recargar, reiniciar analytics o cerrar y volver a iniciar sesión. Cada lector usa su propia conexión y conserva los permisos de Google; nunca usa la cuenta del creador del tablero. No se guardan tokens Google en localStorage/sessionStorage.

Para activar la función después de actualizar: guardar el secreto del cliente en Administración → Conexiones y conectar Google una vez con cada usuario. Las autorizaciones temporales anteriores no contienen refresh token y no se pueden convertir automáticamente. El botón Desconectar elimina la autorización local y solicita revocación a Google. Eliminar un usuario también borra su autorización almacenada.

El cifrado usa Fernet. Opcionalmente configurar `GOOGLE_TOKEN_ENCRYPTION_KEY` en analytics (generar con `python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"`). Si se omite, se deriva una clave específica de Google a partir del `JWT_SECRET` existente, que debe tener al menos 32 caracteres y suficiente aleatoriedad. Mantener estable esa clave: cambiarla invalida secretos/autorizaciones cifrados anteriores; exige volver a guardar el secreto del cliente y reconectar usuarios. Conservarla junto con las copias de seguridad, fuera del repositorio.

Si el proyecto Google está en estado **Testing**, Google vence los refresh tokens de Sheets a los **7 días**. Publicar/verificar la aplicación en Google, según corresponda, elimina esa limitación del modo de prueba; las revocaciones y otras políticas de Google aún pueden exigir reconexión. El sistema informa cuándo hace falta renovar sin cerrar la sesión de la aplicación.
...
Documentación: [modelo de código de Google](https://developers.google.com/identity/oauth2/web/guides/use-code-model), [vencimiento de autorizaciones](https://developers.google.com/identity/protocols/oauth2#expiration).
...
Verificación del 24/09/2026: 90 pruebas de analytics, 19 pruebas dirigidas de Nest y 24 recorridos de navegador aprobados (en dos grupos, con repetición del primer recorrido por espera de carga inicial de Vite). Ambos builds de producción aprobaron. Se cubren renovación concurrente, cifrado, desconexión, separación entre usuarios, inicio/cierre de sesión, filtros sin editar, ceros, valores vacíos, permisos y ausencia de cambios en la configuración guardada. Se revisaron capturas de escritorio y celular. La batería general `npm test` de Nest conserva siete suites con fallos por imports/providers de pruebas antiguas; las cuatro suites indicadas arriba pasan.

Las pruebas usan datos temporales y dobles de Mongo/Google, sin escribir en Atlas ni pedir acceso a una cuenta real. Se conserva la cobertura del Excel de Coparticipación, lectura pública de Sheets, creación de secciones y permisos independientes. La conexión OAuth real debe verificarse tras guardar el secreto en el despliegue: no se configura con credenciales inventadas ni se afirma que una simulación valide la cuenta Google real.
...
La vista de sección incluye filtros por gráfico para cualquier lector autorizado. `POST /v2/workspaces/{id}/widgets/{widget}/chart` admite únicamente filtros adicionales: no permite cambiar hoja, fuente, columnas del gráfico ni filtros guardados. Primero se aplican las restricciones guardadas y después las selecciones del lector. Las sugerencias salen de ese subconjunto, únicamente para campos del gráfico (o todos los campos en una tabla). No modifica MongoDB. Limpiar filtros vuelve a la vista guardada; Actualizar datos conserva los filtros de la visita actual. Las opciones muestran hasta 100 sugerencias y admiten escribir otros valores.
